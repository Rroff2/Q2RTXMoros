...
void gib_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (g_sillygore->value && (g_nodecals->value == 0) && !other->takedamage && level.time > self->delay && (self->count || self->groundentity))
	{
		...
		if (plane && !((other->solid == SOLID_BSP) && (other->s.modelindex != 1)))
		{
			...
			if (!small)
			{
				...
				ent = G_SpawnNC();
				//ent->s.modelindex = gi.modelindex("models/objects/splatter/tris.md2");
				//ent->s.skinnum = rand() % 2;

				ent->s.modelindex = gi.modelindex("models/objects/impacts/tris.md2");
				ent->s.skinnum = 5 + (rand() % 2);
	...
	if (plane) {
		...
		// Rroff trying to get a bit of variation
...
void ThrowGib(edict_t *self, char *gibname, int damage, int type)
{
    ...
    gib->flags |= FL_NO_KNOCKBACK | FL_TOUCH;
...
void ThrowHead(edict_t *self, char *gibname, int damage, int type)
{
	...
	// need to do variants of this for g_sillygore 0 as well
	// if enabled or get shooting heads from gibbed monsters
	//if (g_sillygore->value)
	//{
		self->think = G_FreeEdict;
...
void BecomeExplosion1(edict_t *self)
{
	...
	// Rroff quick and nasty way to try and stop critical keys from being
	// permanently removed.
	...
	if (self->item)
	{
		if (self->item->flags & IT_KEY)
		{
			if (level.sight_client)
				level.sight_client->client->pers.inventory[ITEM_INDEX(self->item)]++;
		}
	}
...
/*QUAKED misc_model (0 .5 .8) (-4 -4 -4) (4 4 4)
Static model - use sparingly as each uses up an entity
"Model" - filename of model i.e. "models/objects/barrels/tris.md2"
*/

// features to add - render effects, animations
...
void giveBasicSupplies(edict_t *self)
{
	gitem_t		*it;
	int			amount, index;
	edict_t		*cl_ent;
	...
	if (it) {
		...
		if (self->client->pers.inventory[index] == 0)
		{
			if (random() <= 0.5)
			{
				self->client->pers.inventory[index] = 1;
				self->client->ps.stats[STAT_PICKUP_ICON] = gi.imageindex(it->icon);
				self->client->ps.stats[STAT_PICKUP_STRING] = CS_ITEMS + index;
				self->client->pickup_msg_time = level.time + 3.0;

				if (self->client->pers.weapon->weapmodel == WEAP_BLASTER)
				{
					self->client->newweapon = it;
					ChangeWeapon(self);
				}
			}
			else {
...
void misc_deadsoldier_playeruse(edict_t *self, edict_t *other)
{
	...
	// players can scavenge for ammo from dead soldiers

	//self->s.renderfx &= ~RF_GLOW;
	if (other->client->pers.mods & MU_SCAVENGER)
	{
		self->playeruse = NULL;

		giveBasicSupplies(other);
...
void SP_misc_deadsoldier(edict_t *ent)
{
	...
	ent->playeruse = misc_deadsoldier_playeruse;
    ent->monsterinfo.aiflags |= AI_GOOD_GUY;

	if (g_moremonsters->value)
		chance = 0.0;

	// Rroff use dead soldiers as a handy reference point for spawning other things
	// keeping in mind they might be under water, in places that aren't accessible, etc.
	// there are a few in bad spots like stuck under a rock on mine3
	if (random() > chance)
	{
		VectorCopy(ent->s.origin, ent->pos1);
		ent->think = spawn_roamer; // let other things spawn first
		ent->nextthink = level.time + FRAMETIME;
	} else {
		// Rroff should we check for visibility to player spawn?
		// so players don't see it tossed and/or add a non-toss spawn version?
		//if (random() < 0.15)
