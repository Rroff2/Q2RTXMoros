...
void button_fire(edict_t *self)
{
	...
	if (!deathmatch->value && self->activator)
	{
		for (ep = event_point_list; ep->name; ep++, i++)
		{
			...
			if (ep->etype == EVENT_INHIBIT_BUTTON) {
				VectorSubtract(self->activator->s.origin, ep->origin, d); // might need to use entity that tried to use as origin
				if (VectorLength(d) < 200) {
					if (level.event_point_enabled[i] == 1) {
						// if the player doesn't have a security tag don't activate the button
						// if they do play an activation sound, disable this inhibit entry and let the button fire
						if (self->activator->client) {
							...
							if (item) {
								...
								if (self->activator->client->pers.inventory[index]) {
									level.event_point_enabled[i] = 0;
									self->activator->client->pers.inventory[index]--;
									gi.sound(self->activator, CHAN_AUTO, gi.soundindex("misc/keyuse.wav"), 1, ATTN_NORM, 0);

									if (!(self->activator->client->anim_priority > ANIM_WAVE))
									{
										if (self->activator->client->ps.pmove.pm_flags & PMF_DUCKED)
										{
											self->activator->client->anim_priority = ANIM_WAVE;
											self->activator->s.frame = FRAME_crattak1 - 1;
											self->activator->client->anim_end = FRAME_crattak3;
										}
										else {

											self->activator->client->anim_priority = ANIM_WAVE;
											self->activator->s.frame = FRAME_point01 - 1;
											self->activator->client->anim_end = FRAME_point06;
										}
									}

									gi.centerprintf(self->activator, "Security tag accepted!");

									if (ep->effect)
									{
										event_effect(self->activator, ep->effect, ep->origin);
									}
							...
							if (level.time > self->touch_debounce_time) {
								//if (level.time < self->touch_debounce_time)
								self->touch_debounce_time = level.time + 3.0;
								gi.centerprintf(self->activator, "%s", ep->msg);
								gi.sound(self->activator, CHAN_AUTO, gi.soundindex("misc/talk1.wav"), 1, ATTN_NORM, 0);
							}
						}
						return;
					}
...
void button_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (!(self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP))
	{
		// Rroff fire a button press event

		if (!self->eventdone) {
			doEvent(other, EVENTFLAG_BUTTON);
			self->eventdone = qtrue;
		}
	}
...
void Touch_DoorTrigger(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// Rroff - see if there are any event points here that spawn monsters
	if (other->client)
	{
		if (!self->eventdone) {
			doEvent(other, EVENTFLAG_DOOR);
			self->eventdone = qtrue;
		}
	}
	...
	if (!deathmatch->value)
	{
		for (ep = event_point_list; ep->name; ep++, i++)
		{
			...
			if (ep->etype == EVENT_INHIBIT_DOOR || ep->etype == EVENT_INHIBIT_DOOR_HEAD) {
				VectorSubtract(other->s.origin, ep->origin, d); // might need to use entity that tried to use door as origin
				if (VectorLength(d) < 200) {
					if (level.event_point_enabled[i] == 1) {
						// if the player doesn't have a security tag don't activate the door
						// if they do play an activation sound, disable this inhibit entry and let the door open
						if (other->client) {
							if (ep->etype == EVENT_INHIBIT_DOOR)
								item = FindItem("Security Tag");
							if (ep->etype == EVENT_INHIBIT_DOOR_HEAD)
							...
							if (item) {
								...
								if (other->client->pers.inventory[index]) {
									...
									gi.sound(other, CHAN_AUTO, gi.soundindex("misc/keyuse.wav"), 1, ATTN_NORM, 0);

									if (ep->effect)
									{
										event_effect(other, ep->effect, ep->origin);
									}
									...
									if (ep->etype == EVENT_INHIBIT_DOOR_HEAD)
									{
										if (!(other->client->anim_priority > ANIM_WAVE))
										{
											if (other->client->ps.pmove.pm_flags & PMF_DUCKED)
											{
												...
												other->client->anim_priority = ANIM_WAVE;
												other->s.frame = FRAME_flip01 - 1;
												other->client->anim_end = FRAME_flip12;
											}
										}

										gi.centerprintf(other, "Biometric scan successful", ep->msg);

										ThrowGib(other, "models/objects/gibs/head2/tris.md2", 80, GIB_ORGANIC);
									}
									if (ep->etype == EVENT_INHIBIT_DOOR)
									{
										if (!(other->client->anim_priority > ANIM_WAVE))
										...
										gi.centerprintf(other, "Security tag accepted!");
										//ThrowGib(other, "models/items/keys/pass/tris.md2", 60, GIB_GENERIC);
									}
									break;
								}
							}
							...
							gi.centerprintf(other, "%s", ep->msg);
