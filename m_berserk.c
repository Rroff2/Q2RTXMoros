...
void berserk_run(edict_t *self)
{
	...
	// horrid hack to reduce berserk flailing around on top of boxes
	// need to check they aren't jumping into something stupid like lava
	/*
	if ((self->enemy) && (self->groundentity) && (infront(self, self->enemy)))
	{
		...
		if (len > 80)
		{
			if (random() < 0.3)
			{
				self->monsterinfo.jumpmove(self);
				self->monsterinfo.last_jump_time = level.time;
				return;
			}
		}
	}*/
...
mframe_t berserk_frames_attack_club[] = {
	{ ai_charge, 29, NULL },
	{ ai_charge, 39, NULL },
	{ ai_charge, 35, berserk_swing },
	{ ai_charge, 21, NULL },
	{ ai_charge, 31, NULL },
	{ ai_charge, 35, NULL },
	{ ai_charge, 28, berserk_attack_club },
	{ ai_charge, 29, NULL }
};
mmove_t berserk_move_attack_club = { FRAME_r_att11, FRAME_r_att18, berserk_frames_attack_club, berserk_run };
...
void berserk_attack_liftoff(edict_t *self)
{
	vec3_t	forward;

	AngleVectors(self->s.angles, forward, NULL, NULL);
	self->s.origin[2] += 1;
	VectorScale(forward, 80, self->velocity);
	self->velocity[2] = 250;
...
{
	//static	vec3_t	aim = {MELEE_DISTANCE, 0, -24};
	static	vec3_t	aim = { MELEE_DISTANCE, 0, -4 };
	qboolean hit;

	hit = fire_hit(self, aim, (35 + (rand() % 10)), 400);
	...
	// TODO: Look at mutant code to perfect landing and attack start/finish
}

void berserk_attack_end(edict_t *self)
{
	self->monsterinfo.aiflags &= ~AI_DUCKED;
	// TODO: Look at mutant code to pefect landing and attack start/finish
}


mframe_t berserk_frames_attack_slam[] =
{
	ai_charge, 0, berserk_attack_liftoff,
	ai_charge, 0, NULL,
	ai_charge, 0, berserk_swing,
	ai_charge, 0, NULL,
	ai_charge, 0, NULL,
	...
	ai_charge, 0, NULL,
	ai_charge, 0, NULL,
	ai_charge, 0, berserk_attack_end,
	ai_charge, 0, NULL
};
...
void berserk_melee(edict_t *self)
{
	...
	// Rroff: "finishing" move if our enemy is within melee range and nearly dead
	// do a small jump slam attack
	...
	if (!self->enemy) // not sure what we are mad at otherwise
	{
		if ((rand() % 2) == 0)
	...
	speed = VectorLength(self->enemy->velocity);
...
mmove_t berserk_move_fall = { FRAME_fall1, FRAME_fall20, berserk_frames_fall, berserk_run };
...
void berserk_pain(edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (self->health < (self->max_health / 2))
	{
		/*if (self->s.skinnum != 1)
		{
			if (self->enemy)
			{
				...
				{
					ent = SelectFarthestDeathmatchSpawnPoint();
					if (ent)
					{
						self->enemy = NULL;
						self->goalentity = ent;
	...
	if (kick > 200)
	{
		self->monsterinfo.currentmove = &berserk_move_fall;
		return;
	}
...
// JUMP
//
void berserk_jump_up(edict_t *self)
{
	vec3_t	forward;
...
/*mframe_t berserk_frames_jump[] =
{
	...
	ai_move, -3, NULL,
	ai_move, 0, NULL,
	ai_move, 8, berserk_jump_up,
	ai_move, 8, NULL,
	ai_move, 12, NULL,
	ai_move, 5, NULL,
	ai_move, 3, NULL
};
mmove_t berserk_move_jump = { FRAME_duck1, FRAME_duck10, berserk_frames_jump, berserk_run };
...
mmove_t berserk_move_jump = { FRAME_duck4, FRAME_duck10, berserk_frames_jump, berserk_run };
...
void berserk_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (!self->deadflag == DEAD_DEAD && attacker)
	{
		VectorSubtract(self->s.origin, attacker->s.origin, v);
		dist = VectorLength(v);

		if (dist <= 100 && attacker->client->pers.tracked_berserk < 10)
		{
			attacker->client->pers.tracked_berserk += 1;
			attacker->client->resp.coop_respawn.tracked_berserk += 1;

			if (attacker->client->pers.tracked_berserk == 10)
			{
				...
				if (attacker->client->pers.inventory[index] < 1)
				{
					...
					gi.cprintf(attacker, PRINT_HIGH, "Activate [Berserk] from the inventory to use it\n");
...
// urgh quick bandaid to try and fix something I seem to have broke
...
void SP_monster_berserk(edict_t *self)
{
	...
	sound_thud = gi.soundindex("tank/thud.wav");
	sound_impact = gi.soundindex("chick/chkatck4.wav");
	...
	self->monsterinfo.jumpmove = berserk_jump;

    self->monsterinfo.currentmove = &berserk_move_stand;
	self->monsterinfo.aiflags2 |= AI2_LADDER;
    self->monsterinfo.scale = MODEL_SCALE;

	self->descname = "Berserker";
