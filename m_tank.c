...
static int  sound_power_up;
static int  sound_bonus;
...
void tank_bonus_up(edict_t *self)
{
	gi.sound(self, CHAN_VOICE, sound_bonus, 1, ATTN_IDLE, 0);
}
...
void tank_bonus(edict_t *self)
{
	edict_t		*ent = NULL;

	self->monsterinfo.bonus_time = level.time;

	//gi.sound(self, CHAN_VOICE, sound_bonus, 1, ATTN_NORM, 0);
	...
	while ((ent = findradius(ent, self->s.origin, 1024)) != NULL) {
		...
		if (ent->health <= 0)
			continue;
		if (ent->enemy) // don't bonus monsters that are angry at us!
		{
			if (ent->enemy == self)
				continue;
		}
		...
		if (ent)
		{
			ent->monsterinfo.bonus_time = level.time;
		}
	}
}

mframe_t tank_frames_bonus[] = {
	{ ai_move, 3,   NULL },
	{ ai_move, 2,   tank_bonus_up },
	{ ai_move, 2,   NULL },
	{ ai_move, 1,   NULL },
	{ ai_move, 6,   NULL },
	{ ai_move, 7,   NULL },
	...
	{ ai_move, 0,   NULL },
	{ ai_move, 0,   NULL },
	{ ai_move, 0,   tank_bonus },
	{ ai_move, -2,  NULL }
};
mmove_t tank_move_bonus = { FRAME_attak201, FRAME_attak217, tank_frames_bonus, tank_run };
...
void tank_pain(edict_t *self, edict_t *other, float kick, int damage)
{
	...
	// Rroff trigger event if health drops below 25%
	if (self->health < (self->max_health * 0.25))
	{
		if (!self->eventdone)
		{
			if (self->s.skinnum > 1) {
				doEvent(self, EVENTFLAG_TANK_25);
				self->eventdone = qtrue;
				self->monsterinfo.power_armor_type = POWER_ARMOR_SCREEN;
				self->monsterinfo.power_armor_power = 400;
				gi.sound(self, CHAN_VOICE, sound_power_up, 1, ATTN_NORM, 0);
			}
		}
	}
...
void TankMachineGun(edict_t *self)
{
	...
	// Rroff bumped the damage up a bit
	
	//monster_fire_bullet(self, start, forward, 20, 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);

    monster_fire_bullet(self, start, forward, 32, 8, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
}
...
void tank_attack(edict_t *self)
{
	...
	// quicker to pull it off the skinnum than string lookup
	if (self->s.skinnum > 1)
	{
		if (level.time > self->monsterinfo.bonus_time + 30 || !self->monsterinfo.bonus_time)
		{
			self->monsterinfo.currentmove = &tank_move_bonus;
			return;
		}
	}
...
void SP_monster_tank(edict_t *self)
{
	...
	sound_power_up = gi.soundindex("misc/power1.wav");
	sound_bonus = gi.soundindex("world/lite_on3.wav");
	...
	self->monsterinfo.attack_range = 1024;
