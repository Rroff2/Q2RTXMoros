...
#define _OFS FOFS
	...
	P(monsterinfo.jumpmove, P_monsterinfo_jumpmove),
	...
	F(monsterinfo.attack_range), // Rroff
	F(monsterinfo.roamer_time), // Rroff
	F(monsterinfo.friendlyfire_time), // Rroff
	F(monsterinfo.last_jump_time), // Rroff

	I(monsterinfo.eventtype),	// Rroff
	L(monsterinfo.eventname),	// Rroff

	F(monsterinfo.chick_teleport_time), // Rroff
	V(monsterinfo.chick_dest),	// Rroff

	I(monsterinfo.pool_health),	// Rroff
	F(monsterinfo.pool_time),		// Rroff
	E(monsterinfo.controlling),		//Rroff

	F(monsterinfo.runaway_time), // Rroff
	FA(monsterinfo.dot_time, MAX_DOTI),
	IA(monsterinfo.dot_amt, MAX_DOTI),
	E(monsterinfo.dot_ent),
	F(monsterinfo.dot_index), // Rroff
	F(monsterinfo.dottick_time), // Rroff

	F(monsterinfo.bonus_time), // Rroff

	I(monsterinfo.surprised_frame), // Rroff

	I(monsterinfo.armoured), // Rroff

	F(monsterinfo.timebomb), // Rroff
	I(monsterinfo.aiflags2),

	I(monsterinfo.grunt_follow), // Rroff
	L(monsterinfo.grunt_name),
	I(monsterinfo.grunt_weapon),
	F(monsterinfo.grunt_firing_time),

	I(monsterinfo.ai_reverse),
	F(monsterinfo.ai_roaming_time),
	F(monsterinfo.ai_last_sight),

	I(monsterinfo.lockdown_wave),
...
#define _OFS LLOFS
	...
	IA(event_point_enabled, MAX_EVENT_POINTS),	// Rroff
...
#define _OFS CLOFS
	...
	I(pers.pool_health), // Rroff
	I(pers.pool_health_max), // Rroff
	...
	I(pers.mods),
	I(pers.player_class),
	I(pers.tracked_reaper),
	I(pers.tracked_berserk),
	I(pers.tracked_gambler),
	I(pers.tracked_edge),
	I(pers.tracked_bouncer),
...
void WriteLevel(const char *filename)
{
    ...
    for (i = 0; i < globals.num_edicts; i++) {
        ...
        if (!ent->inuse)
            continue;
		// Rroff quick and lazy way to avoid rebuilding the reuse index
		// don't save any non-critical entities like gibs
...
void ReadLevel(const char *filename)
{
    ...
    while (1) {
        ...
        if (entnum < 0 || entnum >= game.maxentities) {
			...
			gi.error("%s: bad entity number - %i", __func__, entnum);
        }
		...
		// Rroff - this isn't very well tested
		// freeing disposable entities shouldn't be necessary in theory
		// or a problem but not extensively tested
		...
		if (ent->svflags & SVF_REUSE)
		{
			...
			// Should have a tuned value for max here
			...
			if (!level.max_reuse)
			{
				...
				if (level.reindex > level.max_reuse)
					level.reindex = 0;
			}
		}
