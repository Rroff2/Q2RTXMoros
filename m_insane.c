...
void insane_timer(edict_t *self)
{
	if (self->monsterinfo.timebomb)
		gi.sound(self, CHAN_VOICE, gi.soundindex("weapons/hgrenc1b.wav"), 0.4, ATTN_STATIC, 0);
}
...
mframe_t insane_frames_walk_normal[] =
{
	...
	ai_run,		7,		NULL,
	ai_run,		3.4,	NULL,
	ai_run,		4.6,	insane_timer,
	ai_run,		4.8,	NULL,
	ai_run,		4.4,	NULL,
	ai_run,		8.4,	NULL,
	ai_run,		11.2,	insane_timer,
	ai_run,		6.6,	NULL,
	ai_run,		4.8,	NULL,
	ai_run,		1.8,	NULL,
	ai_run,		0,		NULL
...
void insane_walk(edict_t *self)
{
	...
	if (self->spawnflags & 4)
	{
		...
		if (self->monsterinfo.aiflags & AI_ONAMISSION)
		{
...
void insane_stand(edict_t *self)
{
	...
	else if ((self->spawnflags & 4) && (self->spawnflags & 16)) // If Hold_Ground and Crawl are set
	{
		...
		if ((random() < 0.5) || (self->monsterinfo.aiflags & AI_ONAMISSION))
...
void insane_melee(edict_t *self)
{
	vec3_t		v;

	if ((self->monsterinfo.aiflags & AI_ONAMISSION) && (self->enemy) && (self->enemy->inuse) && (self->enemy->health > 0))
	{
...
void insane_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (self->monsterinfo.timebomb)
	{
		fire_grenade2(self, self->s.origin, vec3_origin, 150, 1, (self->monsterinfo.timebomb - level.time), 120, qtrue);
	}
...
void insane_playeruse(edict_t *self, edict_t *other)
{
	edict_t		*enemy;
	gitem_t		*it;
	int			index;
	...
	if (self->monsterinfo.timebomb)
	{
		gi.cprintf(other, PRINT_HIGH, "Already has a grenade\n");
		return;
	}
	...
	if (it)
	{
		...
		{
			gi.cprintf(other, PRINT_HIGH, "You need a spare grenade\n");
			return;
		}
		if (other->client->pers.weapon &&
			other->client->pers.weapon->tag == AMMO_GRENADES &&
			it->tag == AMMO_GRENADES &&
			other->client->pers.inventory[index] - 1 <= 0) {
			gi.cprintf(other, PRINT_HIGH, "Can't give an equipped grenade\n");
			return;
		}
	...
	self->monsterinfo.aiflags &= ~(AI_STAND_GROUND | AI_COMBAT_POINT);
	self->monsterinfo.aiflags |= AI_ONAMISSION;

	self->monsterinfo.timebomb = level.time + 10;
...
void insane_attack(edict_t *self)
{
...
//  static int skin = 0;    //@@
    ...
    self->monsterinfo.attack = insane_attack;
    self->monsterinfo.melee = insane_melee;
    self->monsterinfo.sight = NULL;
    self->monsterinfo.aiflags |= AI_GOOD_GUY;
	self->playeruse = insane_playeruse;

	self->descname = "Insane";
