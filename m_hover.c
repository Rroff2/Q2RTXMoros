...
// Rroff active defence mode
void hover_ad(edict_t *self)
{
	edict_t		*ent = NULL;
	...
	while ((ent = findradius(ent, self->s.origin, 384)) != NULL) {
		if (!ent->projectile)
			continue;
	...
	if (ent)
	{
		...
		gi.sound(self, CHAN_AUTO, gi.soundindex("weapons/bfg__x1b.wav"), 1, ATTN_NORM, 0);
	}
}
