...
mmove_t soldier_move_runt = { FRAME_runt01, FRAME_runt19, soldier_frames_runt, soldier_run };
...
void soldier_jump_up(edict_t *self)
{
	...
	//VectorScale(forward, 200, self->velocity); // was 200/250
...
{
	ai_move, 0,  NULL, // was 0,0,5
	ai_move, 5,  soldier_jump_up,
	ai_move, 16,  NULL
};*/

mframe_t soldier_frames_jump[] =
{
	ai_move, -4,  NULL, // was 0,0,5
	ai_move, 5,  soldier_jump_up,
	ai_move, 16,  NULL
};
mmove_t soldier_move_jump = { FRAME_duck03, FRAME_duck05, soldier_frames_jump, soldier_run };
	...
	if (self->velocity[2] > 100)
	{
		if (other)
		{
			if (infront(self, other))
			{
				...
				self->monsterinfo.currentmove = &soldier_move_runt;
			}
		}
...
void soldier_fire(edict_t *self, int flash_number)
{
    ...
    if (self->s.skinnum <= 1) {
		...
		if ((flash_number != 7) && (self->groundentity))
		{
			if (self->enemy)
			{
			...
			if ((dist > 160) && (random() < 0.3) && !(self->monsterinfo.aiflags & AI_HOLD_FRAME))
			{
				//monster_fire_grenade(self, start, aim, 50, 600, flash_number);
				fire_grenade(self, start, aim, 50, 600, 2.5, 90);
				gi.sound(self, CHAN_WEAPON, sound_grenade, 1, ATTN_IDLE, 0);
...
void soldier_attack6_refire(edict_t *self)
{
	...
	// try and stop running on the spot attacking
	if (VectorLength(self->velocity) < 50)
		return;
...
void soldier_strafe(edict_t *self)
{
	self->monsterinfo.attack_state = AS_SLIDING;
}

void soldier_nostrafe(edict_t *self)
{
	self->monsterinfo.attack_state = AS_STRAIGHT;
}

mframe_t soldier_frames_attack7[] = {
	{ ai_run, 8, soldier_strafe },
	{ ai_run,  2, NULL },
	{ ai_run, 10, NULL },
	{ ai_run, 9, soldier_fire8 },
	{ ai_run, 11, soldier_fire9 },
	{ ai_run, 16, soldier_fire9 },
	{ ai_run, 13, soldier_fire9 },
	{ ai_run, 12, soldier_fire9 },
	{ ai_run, 9, NULL },
	{ ai_run,  6, NULL },
	{ ai_run, 9, NULL },
	{ ai_run, 10, NULL },
	{ ai_run, 10, NULL },
	{ ai_run, 15, soldier_attack6_refire }
};
...
void soldier_attack(edict_t *self)
{
	...
	if (self->enemy) // should really have one if we got here
	{
		vectoangles(self->enemy->velocity, dir);
		AngleVectors(dir, NULL, right, NULL);
		dist = VectorLength(right);
		dist2 = VectorLength(self->enemy->velocity);
	}

	if ((random() < 0.8) || (dist > 80)) {
		self->monsterinfo.currentmove = &soldier_move_attack7;
		return;
	}

	if ((random() < 0.8) && (dist2 > 200)) {
		self->monsterinfo.currentmove = &soldier_move_attack7;
		return;
	}
...
void SP_monster_soldier_x(edict_t *self)
{
	...
	sound_grenade = gi.soundindex("gunner/gunatck3.wav");
	...
	self->monsterinfo.jumpmove = soldier_jump;
...
void SP_monster_soldier_light(edict_t *self)
{
	...
	self->monsterinfo.attack_range = 700;

    self->s.skinnum = 0;

	self->descname = "Light Recon";
}
...
void SP_monster_soldier(edict_t *self)
{
	...
	self->monsterinfo.attack_range = 500;
