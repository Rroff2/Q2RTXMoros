...
void floater_zap(edict_t *self)
{
	...
	//	G_ProjectSource (self->s.origin, monster_flash_offset[flash_number], forward, right, origin);
		...
		// Rroff later adition for beam length
	...
	*/
	...
	//VectorAdd (origin, self->enemy->s.origin, pos);
	//VectorScale (pos, 0.5, pos);

	// we need to check visibility before doing the attack
	...
	gi.WriteByte(0x6C + (rand() & 3));
	gi.multicast(tr.endpos, MULTICAST_PVS);

	T_Damage(self->enemy, self, self, dir, self->enemy->s.origin, vec3_origin, (25 + rand() % 6), 50, DAMAGE_ENERGY, MOD_FRIED);
}
