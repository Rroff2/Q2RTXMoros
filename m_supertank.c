...
void tank_attak4(edict_t *self)
{
	...
	if (self->s.frame == FRAME_attak4_4)
		VectorSet(offset, 30, 38, 48);
	else
		VectorSet(offset, 30, -38, 48);
	...
	if (self->enemy)
	{
		...
		target[2] += (self->enemy->viewheight * 1.5);
	...
	gi.sound(self, CHAN_WEAPON, gi.soundindex("gunner/gunatck3.wav"), 1, ATTN_NORM, 0);

	fire_grenade(self, start, aim, 50, 700, 2.5, 50 + 40);
}

mframe_t supertank_frames_attack4[] = {
    { ai_move,    -7,  tank_attak4 },
    { ai_move,    -4,  NULL },
    { ai_move,    -4,  NULL },
    { ai_move,    -7,  tank_attak4 },
    { ai_move,    -4,  NULL },
...
{
	// Rroff trigger event if boss health drops below 50%
	if (self->health < (self->max_health / 2))
	{
		if (!(self->s.skinnum == 1))
			doEvent(self, EVENTFLAG_BOSS_50);
...
void supertank_attack(edict_t *self)
{
	...
	// Rroff so we can trigger events when a boss sights a player
	if (!self->eventdone && self->enemy->client) {
		doEvent(self, EVENTFLAG_BOSS_SIGHT);
...
{
	// Rroff trigger an event on boss death
	doEvent(self, EVENTFLAG_BOSS_DIE);
...
//  self->s.sound = gi.soundindex ("bosstank/btkengn1.wav");
	...
	self->descname = "Super Tank";

	self->monsterinfo.attack_range = 4096;
