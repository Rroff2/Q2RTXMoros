...
cvar_t  *g_sillygore;
cvar_t  *g_coopflags;
cvar_t  *g_moremonsters;
cvar_t  *g_hardmonsters;
cvar_t  *g_patchgrunt;
cvar_t  *g_hordemode;
cvar_t  *g_hordecustom;
cvar_t  *g_gibcount;
cvar_t  *sv_maxreuse;
cvar_t  *g_weaptweaks;
cvar_t  *g_nodecals;
...
int PatchPlayerModels(char *modelname)
{
	...
	// Rroff - actors use renamed version - see comment lower down
	// sprintf(outfilename, "%s/players/%s/tris.md2", game->string, modelname);

	sprintf(outfilename, "%s/grunt/%s/tris.md2", game->string, modelname);
	...
	sprintf(outfilename, "%s/grunt", game->string);	// make some dirs if needed
	_mkdir(outfilename);
	sprintf(outfilename, "%s/grunt/%s", game->string, modelname);
	_mkdir(outfilename);
	//sprintf(outfilename, "%s/players/%s/tris.md2", game->string, modelname);
	// Rroff using different path to avoid compatibility problems
...
void InitGame(void)
{
	...
	// Rroff try and make things a bit more random :s
	// though some things seem to depend on things not being too random
	srand(time(NULL));
	...
	g_ejectbrass = gi.cvar("g_ejectbrass", "0", CVAR_ARCHIVE);
	g_healthbars = gi.cvar("g_healthbars", "0", CVAR_ARCHIVE);
	g_doacidfx = gi.cvar("g_doacidfx", "0", CVAR_ARCHIVE);
	g_coophud = gi.cvar("g_coophud", "0", CVAR_ARCHIVE);
	g_sillygore = gi.cvar("g_sillygore", "0", CVAR_ARCHIVE);
	g_coopflags = gi.cvar("g_coopflags", "0", CVAR_ARCHIVE);
	g_moremonsters = gi.cvar("g_moremonsters", "0", CVAR_ARCHIVE);
	g_hardmonsters = gi.cvar("g_hardmonsters", "0", CVAR_ARCHIVE);
	g_patchgrunt = gi.cvar("g_patchgrunt", "1", CVAR_ARCHIVE);
	g_hordemode = gi.cvar("g_hordemode", "1", CVAR_ARCHIVE);
	g_hordecustom = gi.cvar("g_hordecustom", "0", CVAR_ARCHIVE);
	g_gibcount = gi.cvar("g_gibcount", "12", CVAR_ARCHIVE);
	sv_maxreuse = gi.cvar("sv_maxreuse", "0", CVAR_ARCHIVE);
	g_weaptweaks = gi.cvar("g_weaptweaks", "0", CVAR_ARCHIVE);
	g_nodecals = gi.cvar("g_nodecals", "0", CVAR_ARCHIVE);
	...
	// Patch models for misc_grunt

	if (g_patchgrunt->value)
	{
		gi.dprintf("=================================================\n");
		gi.dprintf("== Patching player models for misc_grunt\n");
		gi.dprintf("== Credit: Argh! (Tim Wright)\n");
		gi.dprintf("== Shout out to Argh! for the awesome work on the map tools\n");
		gi.dprintf("== Shout out to David Hyde, Mad Dog and the rest of the Lazarus team\n");
		gi.dprintf("== Using PatchPlayerModels()\n");
		gi.dprintf("== No other code, content or assets from Lazarus used\n");
		gi.dprintf("=================================================\n");

		PatchPlayerModels("male");
		PatchPlayerModels("female");
		PatchPlayerModels("cyborg");
	}
	...
	cl_hudfix = gi.cvar("cl_hudfix", "0", CVAR_USERINFO);
	...
	// probably could be level
	custom_spawn_list = gi.TagMalloc(LK_MAX_CUSTOM * sizeof(custom_spawn_list[0]), TAG_GAME);
...
void ExitLevel(void)
{
	...
	{
		Q_snprintf(command, sizeof(command), "map \"%s\" force\n", level.changemap);
	}
	else
	{
	...
	// Rroff - organise any SVF_REUSE edicts so they don't clutter things up?
...
void G_RunFrame(void)
{
	...
	game.gibcount = g_gibcount->integer;
