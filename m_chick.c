...
void chick_check_teleport(edict_t *self);
...
{
	if (self->monsterinfo.aiflags & AI_CHICK_HUNT)
	{
...
mframe_t chick_frames_walk[] = {
	...
	{ ai_walk, 5,  NULL },
	{ ai_walk, 9,  NULL },
	{ ai_walk, 7,  chick_check_teleport }
};
...
void chick_run(edict_t *self)
{
	...
	// Get here sometimes off the fidget animation, etc. if in hunt mode
	if (!self->enemy)
	{
		self->monsterinfo.currentmove = &chick_move_stand;
		return;
...
void chick_check_teleport(edict_t *self)
{
	if (self->monsterinfo.aiflags & AI_CHICK_HUNT)
	{
		if (level.time > self->monsterinfo.chick_teleport_time) {
			self->monsterinfo.currentmove = &chick_move_duck;
		}
	}
}

edict_t *SelectChickSpawnPoint(edict_t *self)
{
	edict_t		*spot, *spot1;
	edict_t		*ent;
	...
	while ((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) != NULL) {
		...
		// Rroff changed to pos1 to match other stuff

		VectorCopy(spot->pos1, start);
		VectorCopy(spot->pos1, end);

		// move up slightly to avoid the respawn flare in coop
		if (coop->value)
			end[2] += 5;
		else
			end[2] += 1;
		...
		// solid check above and passing origin done to avoid some bugs and before was using a longer end point
		// so needed to get tr.endpost back

		if (!tr.allsolid) {
			VectorCopy(tr.endpos, self->monsterinfo.chick_dest);

			if (level.sight_client && spot) // ocassionally getting crashes here not sure why
			{
				if (visible(spot, level.sight_client))
				{
					return spot;
				}
			}
...
// Currently chick teleports on duck move and when health gets to 50% or lower
// as is once chick starts defensively teleporting will continue to do so every 5 seconds
...
void chick_duck_down(edict_t *self)
{
	...
	if (self->killtarget || self->combattarget || self->deathtarget)
		return; // Rroff don't teleport around if these conditions as it can be annoying

	if (self->item)
	{
		if (self->item->flags & IT_KEY)
			return;
	}

	if (skill->value == 3)
	{
		spawn = SelectChickSpawnPoint(self);

		if (spawn)
		{

			self->monsterinfo.aiflags |= AI_CHICK_HUNT;
			self->monsterinfo.chick_teleport_time = level.time + 5 + (rand() % 3);

			//gi.dprintf("Chick ported\n");
			...
			gi.positioned_sound(self->s.origin, self, CHAN_VOICE, gi.soundindex("misc/tele1.wav"), 1, ATTN_NORM, 0);

			VectorCopy(self->monsterinfo.chick_dest, self->s.old_origin);
			VectorCopy(self->monsterinfo.chick_dest, self->s.origin);
			VectorCopy(spawn->s.angles, self->s.angles);
			self->s.event = EV_OTHER_TELEPORT;
			gi.linkentity(self);
			//gi.dprintf("Chick ported to: %s\n", vtos(self->s.origin));

			gi.WriteByte(svc_temp_entity);
			gi.WriteByte(TE_TELEPORT_EFFECT);
			gi.WritePosition(self->s.origin);
			gi.multicast(self->s.origin, MULTICAST_PVS);
			// gi.sound(self, CHAN_AUTO, gi.soundindex("misc/tele_up.wav"), 1, ATTN_NORM, 0);
			// work around some oddities/things I don't understand how they work
			gi.positioned_sound(self->monsterinfo.chick_dest, self, CHAN_AUTO, gi.soundindex("misc/tele_up.wav"), 1, ATTN_NORM, 0);

			//gi.unlinkentity(self); // does this work or does this killbox the pre-teleport position?
			//KillBox(self);
			//gi.linkentity(self);

			/*if (level.sight_client) // cheaper on performance but will keep moving even if a player there if it isn't the sight client
			{
			...
			// bit of a hack to make teleporting chicks eventually try and find the player(s) again
			// if chick teleports continue to teleport every 10 seconds until spotting the player again?
		}
	}
}
...
// JUMP
//
void chick_jump_up(edict_t *self)
{
	vec3_t	forward;
...
mframe_t chick_frames_jump[] = {
	...
	{ ai_move, -4, NULL },
	{ ai_move, 0, NULL },
	{ ai_move, 12,  chick_jump_up },
	{ ai_move, 12,  NULL },
	{ ai_move, 9,  NULL }
};
mmove_t chick_move_jump = { FRAME_duck01, FRAME_duck06, chick_frames_jump, chick_run };

void chick_jump(edict_t *self)
{
	// check a jump if valid otherwise revert to run
	self->monsterinfo.currentmove = &chick_move_jump;
}

void chick_dodge(edict_t *self, edict_t *attacker, float eta)
{
	// should we check if ground entity?
	/*if (random() > 0.5)
	{
		self->monsterinfo.currentmove = &chick_move_jump;
		return;
...
void SP_monster_chick(edict_t *self)
{
	...
	self->monsterinfo.jumpmove = chick_jump;

	self->monsterinfo.power_armor_type = POWER_ARMOR_SHIELD;
	self->monsterinfo.power_armor_power = 250;

	self->monsterinfo.attack_range = 1280;

	self->descname = "Iron Maiden";

	// self->flags |= FL_IMMUNE_LASER; // Rroff chick is immune to lasers due to power shield
